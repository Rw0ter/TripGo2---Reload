...
**最后更新：2026-06-22**（收尾：首页入口宫格改 4+5 两排居中；README 精简为新机部署快速上手；docs 大体积演示材料 gitignore 不入库。直接提交 master。前次 06-21：四~六轮 AI 深度融合 / 地图 / 首页重设计 / 安卓 APK，PR #85~#93。）
...
**收尾（2026-06-22，直接提交 master）**
1. **首页入口宫格** 由 5 列两排改为第一排 4 个、第二排 5 个、两排水平居中（`home.tsx`，纯排版微调，过 tsc+lint）。
2. **README 精简**：重写为「在新电脑上跑起来」快速上手（后端 `prisma migrate dev` 一步建库+seed、前端 `expo start`），门禁 / AI / 结构各一段。
3. **演示材料不入库**：`docs/*.pptx`、`docs/*.docx`、`docs/_ppt_assets/`（最大 164MB，超 GitHub 100MB 上限）加入 `.gitignore`，按需本地保存。
