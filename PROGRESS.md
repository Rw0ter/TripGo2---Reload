...
- **2026-06-09（续）** 非遗 study 页 + 详情页改版（PR：study-redesign，T6）：主题改为**中国红 + 暖金**（呼应党政红，区别于 App 主体岭南绿）。① 去掉原 8 个 emoji 充当图标的装饰格；study.tsx 改为真实图打底的非遗名录卡（点击进详情）。② **新建 `study/[id].tsx` 非遗详情页**：真实图 hero + 项目简介正文 + 「开始答题」CTA→/quiz。③ 内容**后端驱动**：`seed.ts` 给 8 个 topic 的 content JSON 扩 `{quizId,image,intro}`；image 优先 Legacy 真实非遗照片（粤剧/醒狮/剪纸/皮影=fyxx/*），缺图 4 项（广绣/工夫茶/龙舟/岭南建筑）**联网补**——从 zh.wikipedia 页图 API 取真实图存 `assets/legacy/img/study/*`（来源 Wikimedia）。re-seed 后 /cultural 返回扩展内容。web 导出 DOM 实测：study 列表 10 图全加载 0 占位、详情页 hero 真实图 + 后端简介 + CTA 齐全、0 console 报错。tsc+lint 绿。（注：study/[id] 等动态路由在 `serve` 无 SPA 兜底时会 404，属静态导出表现，dev server 正常。）
- **2026-06-09（续）** 粤语课堂重设计（PR：cantonese-redesign，T7）：核心从「短语表」改为**普通话→粤语翻译器**。① 后端新增 `POST /ai/translate`（DeepSeek 非流式，走统一信封，TRANSLATE_PROMPT 输出地道粤语用字），DTO + 单测（无 key→503 / 有 key→trim 译文）。② 前端 `cantonese.tsx` 重做：文字输入或**原生语音输入**（`expo-speech-recognition`，Web 走浏览器 Web Speech、原生需 dev build + config plugin，已加 app.json 麦克风/语音权限）→ 译成粤语 → 译文 + **expo-speech zh-HK 朗读**；保留常用粤语点读。岭南绿主题。③ `lib/ai.ts` 加 `translateToCantonese`。验证：后端 jest 12/12（含 translate）；live 翻译「吃饭了吗→食咗饭未」「这个多少钱→呢个几钱」；web 预览实测「今天天气很好，我们去喝早茶吧」→「今日天气好好，我哋去饮早茶啦。」0 报错；tsc+lint+export 绿。新增依赖 expo-speech-recognition（含 config plugin，原生语音需 expo prebuild / dev build，非 Expo Go）。
- **2026-06-09（续）** 非遗 study 富化重写（PR：study-rich）：用户反馈首版「太空 / 详情太简陋 / 图标一股 AI 味」。① **后端补内容**：8 个 topic 的 content JSON 扩 `gallery[]`(2–3 张真实图) + `history`(历史渊源段) + `highlights[]`(艺术特色) + `funFact`(冷知识)；图优先 Legacy fyxx 多图（粤剧/醒狮/剪纸/皮影），缺图题材（广绣/工夫茶/龙舟/岭南建筑）从 Wikimedia Commons 补画廊图，共注册 ~17 个新 key。② **重写两页**（三方案竞稿→评审 3 票全选「非遗画卷·编辑式」→合成）：去掉「Ionicon 套彩色圆」模板感，改照片优先 + 朱印/描金/菱形钤印中式质感（中国红 #C8161D + 暖金）。study 列表＝概览横条 + 今日精选大卡 + 冷知识轮换 + 真实图名录瀑布流 + 近期活动 + 测验 CTA；详情＝封面 hero + 历史渊源 + 艺术特色清单 + 多图画廊（Modal 灯箱）+ 「你知道吗」冷知识卡 + 答题 CTA。数据/导航接线全保留。web 预览实测：列表 10 图 0 占位、详情（粤剧 fyxx 3 图画廊 / 龙舟 2 张联网图）0 占位、0 console 报错；tsc+lint+export 绿。
- **2026-06-10** 搜索/文创/粤语三屏独立排版重写（PR：distinct-layouts）：用户反馈这三屏「排版照搬了首页」（绿渐变 Hero→米白圆角上提面板→绿竖条 SectionTitle→FadeInDown 瀑布流）。三屏各自竞稿 3 方案→评审（distinctFromHome / fitsFunction 维度）→合成，改为**贴合各自功能、彼此不同**的版式（仅配色呼应主题）：① **search**＝搜索工具式（搜索框即主角 + 悬浮历史/建议覆盖层 + 热搜榜 TOP10 编号榜 + 猜你想搜 chip）；② **products**＝电商目录式（左侧竖向分类导航栏 + 右侧商品网格 + 综合/销量/价格排序）；③ **cantonese**＝双面板翻译器（普通话输入面板↕粤语译文面板 + 朗读 + 横滑常用语）。各屏功能契约/数据/跳转全保留。web 预览实测：products 39 图 0 占位且左栏(x≈29)/右网格(x≈96)双栏成立；search 结果景点+文创真实图 0 占位；cantonese「你去哪里啊→你去边度啊？」；三屏 0 console 报错；tsc+lint+export 绿。（过程坑：验证中后端进程曾崩溃致浏览器 Failed to fetch，与前端无关，重启 start:dev 后恢复。）
- **2026-06-10** 修 VR 三级全景页「一直显示全景加载中」（PR：vr-panorama-fix）。根因（iframe 内实测定位）：`panorama-html.ts` 的 `THREE.TextureLoader.load(url,onLoad,onProgress,onError)` 回调**错位**——成功回调 onLoad 是空函数，隐藏遮罩的 `loading.done` 被误放在 onProgress（图片加载基本不触发），故即使全景图成功加载、遮罩也永停在「加载中」。修：onLoad 隐藏遮罩、onError 显示失败文案。另：① three.js 由 jsdelivr CDN 改**本地静态** `/static/legacy/three.min.js`（srcDoc iframe 无网/跨域更稳 + 加 THREE 缺失兜底，不再静默崩溃）；② 原 `vr-1/2/3.jpg` 单张 **13–17MB**（也拖慢加载）→ 删除，换 **6 个场景各一张真实等距(2:1) 360° 全景图**（~1.7–2.6MB，Wikimedia Commons 按主题搜近 2:1 大图），`vr.tsx` SCENES 改 per-scene 文件名。web 预览 iframe 实测：three.js + 全景图均加载、遮罩已隐藏(loadingHidden:true)、0 报错。
