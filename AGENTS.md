...
| 文档 | 内容 | 性质 |
|------|------|------|
| `AGENTS.md`（本文件） | 约定、规则、技术栈 | 持久，精简，每会话自动加载 |
...
### 保持精简
`AGENTS.md` 每个会话都加载，写进来的每行都有上下文成本——只放**长期有效的规则**，一次性状态放 `PROGRESS.md`。`PROGRESS.md` 的进度三段要定期剪枝，已完成的老条目合并成一句话即可。
